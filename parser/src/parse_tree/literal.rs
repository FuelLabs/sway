...
impl<'sc> Literal<'sc> {
    pub(crate) fn parse_from_pair(lit: Pair<'sc, Rule>) -> CompileResult<'sc, Self>{
        ...
        let parsed : Result<Literal, CompileError> = match lit_inner.as_rule() {
            Rule::integer => {
                ...
                match rule {
                    Rule::u8_integer => {
                        int_inner.as_str().trim().parse().map(Literal::U8).map_err(|e| {
                    ...
                    Rule::u16_integer => {
                        int_inner.as_str().trim().parse().map(Literal::U16).map_err(|e| {
                            CompileError::Internal(
                                "Called incorrect internal parser on literal type.",
                                int_inner.into_span(),
                            )
                    ...
                    Rule::u32_integer => {
                        int_inner.as_str().trim().parse().map(Literal::U32).map_err(|e| {
                            CompileError::Internal(
                                "Called incorrect internal parser on literal type.",
                                int_inner.into_span(),
                            )
                    ...
                    Rule::u64_integer => {
                        int_inner.as_str().trim().parse().map(Literal::U64).map_err(|e| {
                            CompileError::Internal(
                                "Called incorrect internal parser on literal type.",
                                int_inner.into_span(),
                            )
                    ...
                    Rule::u128_integer => {
                        int_inner.as_str().trim().parse().map(Literal::U128).map_err(|e| {
                            CompileError::Internal(
                                "Called incorrect internal parser on literal type.",
                                int_inner.into_span(),
                            )
