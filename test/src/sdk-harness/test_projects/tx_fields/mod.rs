...
async fn get_contracts() -> (TxContractTest, ContractId, WalletUnlocked) {
    ...
    let mut wallets =
        launch_custom_provider_and_get_wallets(wallet_config, Some(provider_config)).await;
...
async fn generate_coin_predicate_input(
    amount: u64,
    data: Vec<u8>,
...
async fn can_get_input_count() {
    ...
    let result = contract_instance.get_input_count().call().await.unwrap();
...
async fn can_get_output_count() {
    ...
    let result = contract_instance.get_output_count().call().await.unwrap();
...
async fn can_get_input_owner() {
    ...
    let owner_result = contract_instance.get_input_owner(1).call().await.unwrap();
...
async fn can_get_input_amount() {
    ...
    let amount_result = contract_instance.get_input_amount(1).call().await.unwrap();
...
async fn can_get_input_asset() {
    ...
    let asset_result = contract_instance.get_input_asset(1).call().await.unwrap();
...
async fn can_get_input_predicate_length() {
    let (contract_instance, _, wallet) = get_contracts().await;
    let provider = wallet.get_provider().unwrap();
    let (predicate_bytecode, predicate_coin) =
        generate_coin_predicate_input(100, vec![], &wallet).await;
...
async fn can_get_input_predicate_data_length() {
    ...
    let (_, predicate_coin) =
        generate_coin_predicate_input(100, predicate_data.clone(), &wallet).await;
...
async fn can_get_input_predicate_data() {
    let (contract_instance, _, wallet) = get_contracts().await;
    let provider = wallet.get_provider().unwrap();
    let predicate_data_bytes: [u8; 32] =
        Bytes32::from_str("49a7e58dc8b6397a25de9090cb50e71c2588c773487d1da7066d0c7198c567c0")
            .unwrap()
            .into();
    let predicate_data = predicate_data_bytes.to_vec();
    let (_, predicate_coin) =
...
async fn can_get_output_type() {
    let (contract_instance, _, _) = get_contracts().await;
    let result = contract_instance.get_output_type(0).call().await.unwrap();
    assert_eq!(result.value, Output::Contract());

    let result = contract_instance.get_output_type(1).call().await.unwrap();
