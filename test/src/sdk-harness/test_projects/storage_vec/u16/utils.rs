    ...
    pub async fn get_contract_instance() -> (MyContract, ContractId) {
        ...
        let id = Contract::deploy(
            ...
            StorageConfiguration::with_storage_path(Some(
                "test_artifacts/storage_vec/svec_u16/out/debug/svec_u16-storage_slots.json".to_string(),
            )),
        )
        .await
        .unwrap();
...
pub mod wrappers {
    ...
    pub async fn push(instance: &MyContract, value: u16) {
        instance.u16_push(value).call().await.unwrap();
    }

    pub async fn get(instance: &MyContract, index: u64) -> u16 {
        instance.u16_get(index).call().await.unwrap().value
    }

    pub async fn pop(instance: &MyContract) -> u16 {
        instance.u16_pop().call().await.unwrap().value
    }

    pub async fn remove(instance: &MyContract, index: u64) -> u16 {
        instance.u16_remove(index).call().await.unwrap().value
    }
    ...
    pub async fn len(instance: &MyContract) -> u64 {
        instance.u16_len().call().await.unwrap().value
    }

    pub async fn is_empty(instance: &MyContract) -> bool {
        instance.u16_is_empty().call().await.unwrap().value
    }

    pub async fn clear(instance: &MyContract) {
        instance.u16_clear().call().await.unwrap();
    }
}
