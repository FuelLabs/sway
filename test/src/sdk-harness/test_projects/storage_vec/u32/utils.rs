    ...
    pub async fn get_contract_instance() -> (MyContract, ContractId) {
        ...
        let id = Contract::deploy(
            ...
            StorageConfiguration::with_storage_path(Some(
                "test_artifacts/storage_vec/svec_u32/out/debug/svec_u32-storage_slots.json".to_string(),
            )),
        )
        .await
        .unwrap();
...
pub mod wrappers {
    ...
    pub async fn push(instance: &MyContract, value: u32) {
        instance.u32_push(value).call().await.unwrap();
    }

    pub async fn get(instance: &MyContract, index: u64) -> u32 {
        instance.u32_get(index).call().await.unwrap().value
    }

    pub async fn pop(instance: &MyContract) -> u32 {
        instance.u32_pop().call().await.unwrap().value
    }

    pub async fn remove(instance: &MyContract, index: u64) -> u32 {
        instance.u32_remove(index).call().await.unwrap().value
    }

    pub async fn swap_remove(instance: &MyContract, index: u64) -> u32 {
        instance.u32_swap_remove(index).call().await.unwrap().value
    }

    pub async fn insert(instance: &MyContract, index: u64, value: u32) {
        instance.u32_insert(index, value).call().await.unwrap();
    }

    pub async fn len(instance: &MyContract) -> u64 {
        instance.u32_len().call().await.unwrap().value
    }

    pub async fn is_empty(instance: &MyContract) -> bool {
        instance.u32_is_empty().call().await.unwrap().value
    }

    pub async fn clear(instance: &MyContract) {
        instance.u32_clear().call().await.unwrap();
    }
}
