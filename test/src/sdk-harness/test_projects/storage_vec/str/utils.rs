...
pub mod wrappers {
    ...
    pub async fn push(instance: &MyContract, value: String) {
        instance.str_push(value.try_into().unwrap()).call().await.unwrap();
    }
    ...
    pub async fn set(instance: &MyContract, index: u64, value: String) {
        instance.str_set(index, value.try_into().unwrap()).call().await.unwrap();
    }

    pub async fn insert(instance: &MyContract, index: u64, value: String) {
        instance.str_insert(index, value.try_into().unwrap()).call().await.unwrap();
    }
