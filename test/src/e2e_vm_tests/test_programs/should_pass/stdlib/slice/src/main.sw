script;

use std::{intrinsics::{size_of, size_of_val}};
use core::slice::from_parts;
use core::slice::*;

fn addr_of<U>(val: U) -> __ptr[u64] {
    asm(r1: __addr_of(val)) { r1: __ptr[u64] }
}

struct TestStruct {
    boo: bool,
    uwu: u64,
}

fn main() /*-> __slice[u64]*/ {
    // Create a struct
    let foo = TestStruct {
        boo: true,
        uwu: 42,
    };
    let foo_len = size_of_val(foo);
    assert(foo_len == 16);

    // Get a slice to it
    let foo_ptr: __ptr[u64] = addr_of(foo);
    let buf_len = foo_len / size_of::<u64>();
    let a = 0;
    let foo_buf: __slice[u64] = from_parts::<u64>(foo_ptr, buf_len);
    assert(foo_buf.ptr() == foo_ptr);
    assert(foo_buf.len() == 2);

    // // Convert to a vector
    // let foo_vec: Vec<u64> = Vec::<u64>::from(foo_buf);
    // assert(foo_vec.len() == 2);

    // // Return it
    // foo_vec.as_slice()
}
