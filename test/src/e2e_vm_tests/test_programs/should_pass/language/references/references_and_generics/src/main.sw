...
impl<T> Eq for S<T> where T: Eq {
    fn eq(self, other: Self) -> bool {
        ...
        let self_r_mut_t_ptr = asm(r: self.r_mut_t) { r: raw_ptr };
        let self_r_mut_r_mut_t_ptr = asm(r: self.r_mut_r_mut_t) { r: raw_ptr };

        let other_r_t_ptr = asm(r: other.r_t) { r: raw_ptr };
        let other_r_r_t_ptr = asm(r: other.r_r_t) { r: raw_ptr };
        let other_r_mut_t_ptr = asm(r: other.r_mut_t) { r: raw_ptr };
        let other_r_mut_r_mut_t_ptr = asm(r: other.r_mut_r_mut_t) { r: raw_ptr };
        ...
        &&
        self_r_mut_t_ptr.read::<T>() == other_r_mut_t_ptr.read::<T>()
        &&
        self_r_mut_r_mut_t_ptr.read::<raw_ptr>().read::<T>() == other_r_mut_r_mut_t_ptr.read::<raw_ptr>().read::<T>()
    }
}
...
fn test<T>(s: S<T>, v: T) where T: Eq {
    ...
    let s_r_mut_t_ptr = asm(r: s.r_mut_t) { r: raw_ptr };
    let s_r_mut_r_mut_t_ptr = asm(r: s.r_mut_r_mut_t) { r: raw_ptr };
...
fn main() -> u64 {
    ...
    let mut s_x = S { r_t: &x, r_r_t: & &x, r_mut_t: &mut x, r_mut_r_mut_t: &mut &mut x };
    test(s_x, x);
    
    let s_s_x = S { r_t: &s_x, r_r_t: & &s_x, r_mut_t: &mut s_x, r_mut_r_mut_t: &mut &mut s_x };
    test(s_s_x, s_x);

    let mut a = A { x: 321 };
    
    let s_a = S { r_t: &a, r_r_t: & &a, r_mut_t: &mut a, r_mut_r_mut_t: &mut &mut a };
    test(s_a, a);
