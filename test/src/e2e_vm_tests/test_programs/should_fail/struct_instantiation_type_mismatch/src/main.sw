...
fn main() {
    ...
    // ---------------------------------------------
    let _: MyStruct = Struct {
        x: Option::Some(123),
    };

    let _: MyOtherStruct = Struct {
        x: Option::Some(123),
    };

    let s: StructAlias<Option<u8>> = Struct {
        x: Option::Some(123u64),
    };
    match s.x {
    ...
    let s: Struct<Option<u8>> = StructAlias {
        x: Option::Some(123u64),
    };
    match s.x {
        Some(x) => x == 123u8, // No error here.
    ...
    let s: Struct<Option<u8>> = Struct::<Option<bool>> {
        x: Option::Some(true),
    };
    match s.x {
    ...
    let s: GenericStruct<_, bool> = GenericStruct::<u8, _> {
        a: 123u64,
        b: true,
    };
    let _ = s.a == 123u8; // No error here.
    let _ = s.b == true; // No error here.
    let s: GenericStruct<u8, bool> = GenericStruct::<u8, u32> {
        a: 123,
        b: true,
    };
    let _ = s.a == 123u8; // No error here.
