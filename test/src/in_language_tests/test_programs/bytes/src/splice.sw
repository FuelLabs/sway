...
fn bytes_splice() {
    ...
    assert_eq(bytes.get(1).unwrap(), 13u8);

    // spliced => [7, 9, 11]
    assert_eq(spliced.len(), 3);
    assert_eq(spliced.get(0).unwrap(), b);
    assert_eq(spliced.get(1).unwrap(), c);
    assert_eq(spliced.get(2).unwrap(), 11u8);
}
...
fn bytes_splice_front() {
    ...
    assert_eq(spliced.get(0).unwrap(), a);
    assert_eq(spliced.get(1).unwrap(), b);
}
...
fn bytes_splice_entire_range() {
    ...
    assert_eq(spliced.get(0).unwrap(), a);
    assert_eq(spliced.get(1).unwrap(), b);
    assert_eq(spliced.get(2).unwrap(), c);
}
...
fn bytes_splice_replace_smaller() {
    ...
    assert_eq(bytes.len(), 3);
    assert_eq(bytes.get(0).unwrap(), a);
    assert_eq(bytes.get(1).unwrap(), 42u8);
    assert_eq(bytes.get(2).unwrap(), 13u8);
...
fn bytes_splice_replace_larger() {
    ...
    assert_eq(bytes.get(0).unwrap(), a);
    assert_eq(bytes.get(1).unwrap(), 42u8);
    assert_eq(bytes.get(2).unwrap(), 50u8);
    assert_eq(bytes.get(3).unwrap(), 60u8);
...
fn bytes_splice_replace_overlap() {
    ...
    assert_eq(bytes.get(0).unwrap(), 10u8);
    assert_eq(bytes.get(1).unwrap(), 12u8);
