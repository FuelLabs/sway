...
fn result_unwrap_or() {
    ...
    assert_eq(err_1.unwrap_or(10u64), 10u64);
}
...
#[test]
fn result_expect_non_trivial_error() {
    let ok_1: Result<u64, u64> = Ok(0u64);
    assert_eq(ok_1.expect(ExpectError::Test), 0u64);
}

#[test(should_revert)]
fn revert_result_expect_non_trivial_error_when_err() {
    let err_1: Result<u64, u64> = Err(0u64);
    let _result = err_1.expect(ExpectError::Test);
}
