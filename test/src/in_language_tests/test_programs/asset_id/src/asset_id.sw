...
fn asset_id_default() {
    ...
    let asset_id_test_abi = abi(AssetIdAbi, contract_id.bits());

    let result_asset_id = asset_id_test_abi.default_asset_id();
    let computed_asset_id = AssetId::new(contract_id, SubId::zero());

    assert_eq(result_asset_id, computed_asset_id);
}

// Inline (context-independent) tests for the `AssetId` type.
#[test]
fn asset_id_hasher() {
...
fn asset_id_eq() {
    ...
    assert_eq(asset_1, asset_2);
    assert_eq(asset_3, asset_4);
    assert_eq(asset_5, asset_6);
}
...
fn asset_id_ne() {
    ...
    assert_ne(asset_1, asset_3);
    assert_ne(asset_1, asset_4);
    assert_ne(asset_1, asset_5);
    assert_ne(asset_1, asset_6);
    assert_ne(asset_2, asset_3);
    assert_ne(asset_2, asset_4);
    assert_ne(asset_2, asset_5);
    assert_ne(asset_2, asset_6);
    assert_ne(asset_3, asset_5);
    assert_ne(asset_3, asset_6);
}
...
fn asset_id_new() {
    ...
    assert_eq(
        asset_2
            .bits(),
        0x58e8f2a1f78f0a591feb75aebecaaa81076e4290894b1c445cc32953604db089,
    );
    ...
    assert_eq(
        asset_4
            .bits(),
        0xa5de9b714accd8afaaabf1cbd6e1014c9d07ff95c2ae154d91ec68485b31e7b5,
    );
    ...
    assert_eq(
        asset_5
            .bits(),
        0xbba91ca85dc914b2ec3efb9e16e7267bf9193b14350d20fba8a8b406730ae30a,
    );
    ...
    assert_eq(
        asset_6
            .bits(),
        0x8667e718294e9e0df1d30600ba3eeb201f764aad2dad72748643e4a285e1d1f7,
    );
    ...
    assert_eq(
        asset_7
            .bits(),
        0x4ab9077c34a6903bc59693414a4fe8ccf275d93f2daacd849574933737c27757,
    );
    ...
    assert_eq(
        asset_8
            .bits(),
        0x2a9bb11102924faefcdbd39baa7858c5f5e49ed2a4205f6759c4a8648bee2942,
    );
}

#[test]
fn asset_id_base() {
    // `AssetId::base()` returns the chain's base (consensus) asset id, which is
    // the zero asset id in the default test environment.
...
fn asset_id_try_from_bytes() {
    ...
    assert_eq(asset_id_6.unwrap(), AssetId::zero());
    bytes_6.set(0, 255u8);
    assert_eq(asset_id_6.unwrap(), AssetId::zero());
}
