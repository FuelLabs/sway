...
fn alloc_alloc() {
    ...
    assert_eq(u8_ptr.read::<u8>(), 0u8);
    u8_ptr.write(u8::max());
    assert_eq(u8_ptr.read::<u8>(), u8::max());

    // Can alloc u64
    let u64_ptr = alloc::<u64>(1);
    assert_eq(u64_ptr.read::<u64>(), 0u64);
    u64_ptr.write(u64::max());
    assert_eq(u64_ptr.read::<u64>(), u64::max());

    // Can alloc b256
    let b256_ptr = alloc::<b256>(1);
    assert_eq(b256_ptr.read::<b256>(), b256::zero());
    b256_ptr.write(b256::max());
    assert_eq(b256_ptr.read::<b256>(), b256::max());

    // Can alloc struct
    let address_ptr = alloc::<Address>(1);
    assert_eq(address_ptr.read::<Address>(), Address::zero());
    address_ptr.write(Address::from(b256::max()));
    assert_eq(address_ptr.read::<Address>(), Address::from(b256::max()));
    ...
    let mut iter = 0;
    while iter < count {
        assert_eq(multiple_u64.add::<u64>(iter).read::<u64>(), 0u64);
        iter += 1;
    }
    ...
    // assert_eq(array_ptr.read::<u64>(), 0u64);
    // assert_eq(array_ptr.add::<u64>(1).read::<u64>(), 0u64);
    // array_ptr.write(u64::max());
    // array_ptr.add::<u64>(1).write(u64::max());
    // assert_eq(array_ptr.read::<u64>(), u64::max());
    // assert_eq(array_ptr.add::<u64>(1).read::<u64>(), u64::max());
}
...
fn alloc_realloc() {
    ...
    assert_eq(realloc_zero_ptr.read::<u64>(), u64::zero());

    // Can realloc u8
    let u8_ptr = alloc::<u8>(1);
    u8_ptr.write(u8::max());
    let realloc_u8_ptr = realloc::<u8>(u8_ptr, 1, 2);
    assert_eq(realloc_u8_ptr.read::<u8>(), u8::max());
    assert_eq(realloc_u8_ptr.add::<u8>(1).read::<u8>(), u8::zero());

    // Can alloc u64
    let u64_ptr = alloc::<u64>(1);
    u64_ptr.write(u64::max());
    let realloc_u64_ptr = realloc::<u64>(u64_ptr, 1, 2);
    assert_eq(realloc_u64_ptr.read::<u64>(), u64::max());
    assert_eq(realloc_u64_ptr.add::<u64>(1).read::<u64>(), u64::zero());

    // Can alloc b256
    let b256_ptr = alloc::<b256>(1);
    b256_ptr.write(b256::max());
    let realloc_b256_ptr = realloc::<b256>(b256_ptr, 1, 2);
    assert_eq(realloc_b256_ptr.read::<b256>(), b256::max());
    assert_eq(realloc_b256_ptr.add::<b256>(1).read::<b256>(), b256::zero());
    ...
    // assert_eq(realloc_array_ptr.read::<u64>(), u64::max());
    // assert_eq(realloc_array_ptr.add::<u64>(1).read::<u64>(), u64::max());
    // assert_eq(realloc_array_ptr.add::<u64>(2).read::<u64>(), u64::zero());
    ...
    let mut iter3 = count;
    while iter3 < recount {
        assert_eq(realloc_multiple_u64.add::<u64>(iter3).read::<u64>(), 0u64);
        iter3 += 1;
    }
    ...
    assert_eq(same_realloc_u64_ptr.read::<u64>(), u64::max());
    assert_eq(
        same_realloc_u64_ptr
            .add::<u64>(1)
            .read::<u64>(),
        u64::zero(),
    );
    ...
    assert_eq(less_realloc_u64_ptr.read::<u64>(), u64::max());
    assert_eq(
        less_realloc_u64_ptr
            .add::<u64>(1)
            .read::<u64>(),
        u64::zero(),
    );
    ...
    assert_eq(exclude_realloc_u64_ptr.read::<u64>(), u64::max());
    assert_eq(
        exclude_realloc_u64_ptr
            .add::<u64>(1)
            .read::<u64>(),
        u64::zero(),
    );
...
fn alloc_realloc_bytes() {
    ...
    assert_eq(realloc_zero_ptr.read::<u8>(), u8::zero());
