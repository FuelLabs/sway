...
// check: $(s_var=$VAL) = get_local __ptr string<10>, $anon_var
// check: $(str_lit=$VAL) = const string<10> "foo \x5ct bar"
// check: store $str_lit to $s_var

// check: $(s_val=$VAL) = load $s_var
// check: $(s_var=$VAL) = get_local __ptr string<10>, s
// check: store $s_val to $s_var

// check: $(l_var=$VAL) = get_local __ptr string<10>, s
// check: $(l_val=$VAL) = load $l_var
// check: $(r_var=$VAL) = get_local __ptr string<10>, s
