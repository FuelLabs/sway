...
fn check_test_forc_tomls(all_tests_dir: &Path) -> Result<()> {
    ...
    fn check_std_dependency(toml: &Table) -> Result<()> {
        return if let Some(implicit_std) = toml.get("project").and_then(|v| v.get("implicit-std")) {
            match implicit_std.as_bool() {
                ...
                Some(false) => Ok(()),
                _ => Err(anyhow!("'project.implicit-std' value is invalid: `{}`. In tests 'project.implicit-std' must be set to `false`.", implicit_std.to_string())),
            }
        } else {
            ...
            let is_local_import = lib
                ...
                .map(|path| {
                    ...
                    path.ends_with(&format!("../../sway-lib-{lib_name}"))
                        || path.contains(&format!("../../{REDUCED_STD_LIBS_DIR_NAME}/sway-lib-std-"))
                })
                .unwrap_or_default();
    ...
    fn check_project_authors_field(toml: &Table) -> Result<()> {
        ...
        if let Some(field) = toml.get("project").and_then(|v| v.get("authors")) {
            let err = |field: &Value| {
                Err(anyhow!("'project.authors' value is invalid: `{}`. 'project.authors' field is mandatory and must be set to `[\"{AUTHOR}\"]`.", field.to_string()))
            };
    ...
    fn check_mandatory_project_field(
        ...
        if let Some(field) = toml.get("project").and_then(|v| v.get(field_name)) {
            match field.as_str() {
                ...
                _ => Err(anyhow!("'project.{field_name}' value is invalid: `{}`. 'project.{field_name}' field is mandatory and must be set to `\"{field_value}\"`.", field.to_string())),
            }
        } else {
    ...
    fn find_test_forc_tomls(path: &Path, forc_tomls: &mut Vec<PathBuf>) {
        if path.is_dir() {
            for entry in std::fs::read_dir(path).unwrap() {
                ...
                if entry.path().to_str().unwrap().contains(REDUCED_STD_LIBS_DIR_NAME) {
                    continue;
                }
