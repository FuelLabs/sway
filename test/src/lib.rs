...
use anyhow::Result;
use clap::Parser;
use e2e_vm_tests::{FilterConfig, RunConfig};
...
pub struct Cli {
    ...
    pub print_ir: Option<Vec<String>>,
    ...
    pub print_asm: Option<Vec<String>>,
...
pub fn configs_from_cli(cli: &Cli) -> (FilterConfig, RunConfig) {
    let filter_config = FilterConfig {
        include: cli.include.clone(),
        exclude: cli.exclude.clone(),
        skip_until: cli.skip_until.clone(),
        ...
        snapshot_only: cli.snapshot_only,
    };
    ...
    (filter_config, run_config)
}

pub async fn run(cli: Cli) -> Result<()> {
    init_tracing_subscriber(Default::default());

    let (filter_config, run_config) = configs_from_cli(&cli);
    ...
    // Run IR tests
    if !filter_config.first_only && !filter_config.snapshot_only {
        println!("\n");
        ir_generation::run(
