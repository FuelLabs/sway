...
use fuel_tx::{ContractId, AssetId};
...
async fn setup() {
    ...
    let compiled_escrow = Contract::load_sway_contract("./out/debug/escrow.bin").unwrap();
    let compiled_asset = Contract::load_sway_contract("./tests/artifacts/asset/out/debug/asset.bin").unwrap();

    let num_wallets = 5;
    let coins_per_wallet = 0;
    let amount_per_coin = 0;
