...
impl SourceEngine {
    ...
    pub fn is_source_id_autogenerated(&self, source_id: &SourceId) -> bool {
        self.get_path(source_id).display().to_string().contains("<autogenerated>")
    }
    ...
    pub fn get_associated_autogenerated_source_id(&self, source_id: &SourceId) -> SourceId {
        let path = self.get_path(source_id);
        let file_name = PathBuf::from_str(path.file_name().unwrap().to_str().unwrap()).unwrap();
        ...
        self.get_source_id_with_program_id(&path, source_id.program_id())
    }
