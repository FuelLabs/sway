...
impl Bytes {
    ...
    pub fn push(ref mut self, byte: u8) {
        ...
        // Write `byte` after the last element
        self.buf.write(byte, self.len);
    ...
    pub fn pop(ref mut self) -> Option<u8> {
        ...
        let byte = self.buf.read(self.len);
    ...
    pub fn insert(ref mut self, index: u64, element: u8) {
        ...
        let index_ptr = start.add(index);

        // Shift everything over to make space.
        let mut i = self.len;
        while i > index {
            let idx_ptr = start.add(i);
            let previous = idx_ptr.sub(1);
            previous.copy_to(idx_ptr, 1);
            i -= 1;
        }
    ...
    pub fn remove(ref mut self, index: u64) -> u8 {
        ...
        let item_ptr = start.add(index);
        // Read the value at `index`
        let ret = item_ptr.read();

        // Shift everything down to fill in that spot.
        let mut i = index;
        while i < self.len {
            let idx_ptr = start.add(i);
            let next = idx_ptr.add(1);
            next.copy_to(idx_ptr, 1);
            i += 1;
        }
    ...
    pub fn swap(ref mut self, element1_index: u64, element2_index: u64) {
        ...
        let element1_ptr = self.buf.ptr().add(element1_index);
        let element2_ptr = self.buf.ptr().add(element2_index);

        let element1_val = self.buf.read(element1_index);
        element2_ptr.copy_to(element1_ptr, 1);
        self.buf.write(element1_val, element2_index);
    }
...
impl Bytes {
    ...
    pub fn split_at(self, mid: u64) -> (Self, Self) {
        ...
        let mut left_bytes = Self { buf: raw_slice::alloc(left_len), len: left_len };
        let mut right_bytes = Self { buf: raw_slice::alloc(right_len), len: right_len };

        if mid > 0 {
            self.buf.ptr().copy_to(left_bytes.buf.ptr(), left_len);
        };
        if mid != self.len {
            self.buf.ptr().add(mid).copy_to(right_bytes.buf.ptr(), right_len);
        };
    ...
    pub fn append(ref mut self, ref mut other: self) {
        ...
        self.buf.resize(both_len);

        let mut i = 0;
        while i < other.len {
            let new_ptr = self.buf.ptr().add(other_start);
            new_ptr.add(i).write(other.buf.ptr().add(i).read());
            i += 1;
        }
...
impl From<b256> for Bytes {
    fn from(b: b256) -> Self {
        ...
        __addr_of(b).copy_to(bytes.buf.ptr(), 32);
    ...
    fn into(self) -> b256 {
        ...
        self.buf.ptr().copy_to_t::<b256>(ptr, 1);
...
fn test_as_raw_slice() {
    ...
    assert(slice_1.len() == slice_2.len());
}
