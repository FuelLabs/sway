    ...
    pub fn from_predicate_data_by_index(predicate_index: u64) -> BufferReader {
        match __gtf::<u8>(predicate_index, GTF_INPUT_TYPE) {
            ...
            INPUT_READ_ONLY_DATA_COIN_PREDICATE => {
                let ptr = __gtf::<raw_ptr>(predicate_index, GTF_INPUT_DATA_COIN_PREDICATE_DATA);
                let _len = __gtf::<u64>(predicate_index, GTF_INPUT_DATA_COIN_PREDICATE_DATA_LENGTH);
                BufferReader { ptr }
            },
            _ => __revert(0),
        }
    ...
    pub fn from_data_coin_data_by_index (data_coin_index: u64) -> BufferReader {
        match __gtf::<u8>(data_coin_index, GTF_INPUT_TYPE) {
            ...
            INPUT_READ_ONLY_DATA_COIN => {
                let ptr = __gtf::<raw_ptr>(data_coin_index, GTF_INPUT_COIN_DATA);
                let _len = __gtf::<u64>(data_coin_index, GTF_INPUT_COIN_DATA_LENGTH);
                BufferReader { ptr }
            },
            INPUT_READ_ONLY_DATA_COIN_PREDICATE => {
                let ptr = __gtf::<raw_ptr>(data_coin_index, GTF_INPUT_DATA_COIN_PREDICATE_DATA);
                let _len = __gtf::<u64>(data_coin_index, GTF_INPUT_DATA_COIN_PREDICATE_DATA_LENGTH);
                BufferReader { ptr }
            },
            _ => __revert(0),
        }
