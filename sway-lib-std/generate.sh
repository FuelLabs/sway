...
END=64
for ((i=END;i>=START;i--)); do
    CODE="#[cfg(experimental_const_generics = false)]\nimpl<T> AbiDecode for [T; $i] where T: AbiDecode { fn is_memcopy() -> bool { __encode_memcopy::<Self>() }\nfn abi_decode(ref mut buffer: BufferReader) -> [T; $i] { let first: T = buffer.decode::<T>(); let mut array = [first; $i]; let mut i = 1; while i < $i { array[i] = buffer.decode::<T>(); i += 1; }; array } }"
...
END=64
for ((i=END;i>=START;i--)); do
    CODE="#[cfg(experimental_const_generics = false)]\nimpl AbiDecode for str[$i] { fn is_memcopy() -> bool { __encode_memcopy::<Self>() }\nfn abi_decode(ref mut buffer: BufferReader) -> str[$i] { let data = buffer.read_bytes($i); asm(s: data.ptr()) { s: str[$i] } } }"
...
generate_tuple_encode() {
    ...
    CODE="$CODE{ fn is_memcopy() -> bool { __encode_memcopy::<Self>() }"
