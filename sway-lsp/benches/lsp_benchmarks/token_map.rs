...
fn benchmarks(c: &mut Criterion) {
    ...
    c.bench_function("idents_at_position", |b| {
        b.iter(|| session.token_map().idents_at_position(position, session.token_map().iter()))
    });

    c.bench_function("tokens_at_position", |b| {
        b.iter(|| session.token_map().tokens_at_position(engines.se(), &uri, position, None))
    });

    c.bench_function("token_at_position", |b| {
        b.iter(|| session.token_map().token_at_position(engines.se(), &uri, position))
    });

    c.bench_function("parent_decl_at_position", |b| {
        b.iter(|| session.token_map().parent_decl_at_position(engines.se(), &uri, position))
    });
}
