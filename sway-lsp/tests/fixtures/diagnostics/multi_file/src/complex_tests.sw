...
pub fn enum_match_exp_bugfix_test() {
    let a: Result<MyIdentity, MyAuthError> = Ok(MyIdentity::Address(MyAddress { inner: 7 }));
    ...
    // should fail with non-exhaustive
    let d = match a {
        Ok(MyIdentity::Address(MyAddress {
            inner: ORACLE_1,
        })) => 1,
...
pub fn enum_match_exp_bugfix_test2() {
    let a: Result<MyIdentity, MyAuthError> = Ok(MyIdentity::ContractId(MyContractId { inner: false }));
