...
async fn init_and_open(service: &mut LspService<GlobalState>, entry_point: PathBuf) -> Url {
...
async fn shutdown_and_exit(service: &mut LspService<GlobalState>) {
...
async fn write_all_example_asts() {
    let (mut service, _) = LspService::build(GlobalState::new)
...
#[tokio::test]
async fn initialize() {
    let (mut service, _) = LspService::new(GlobalState::new);
    let _ = lsp::initialize_request(&mut service).await;
}
...
async fn publish_diagnostics_dead_code_warning() {
    let (mut service, socket) = LspService::new(GlobalState::new);
