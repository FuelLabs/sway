...
impl ServerStateExt {
    ...
    fn next_event(&self, inbox: &Receiver<lsp_server::Message>) -> Option<Event> {
        select! {
            ...
            recv(self.event_loop_state.task_pool.receiver) -> task =>
    ...
    fn handle_event(&mut self, event: Event) -> Result<()> {
        ...
        if tracing::enabled!(tracing::Level::INFO) {
            let task_queue_len = self.event_loop_state.task_pool.handle.len();
            if task_queue_len > 0 {
        ...
        match event {
            ...
            Event::Task(task) => {
                ...
                while let Ok(task) = self.event_loop_state.task_pool.receiver.try_recv() {
                    self.handle_task(task);
                }
            }
        }
    ...
    fn on_request(&mut self, req: Request) {
        ...
        dispatcher.on_sync_mut::<lsp_types::request::Shutdown>(|s, ()| {
            s.event_loop_state.shutdown_requested = true;
            tracing::info!("Shutting Down the Sway Language Server");

            let _ = s.state.sessions.iter().map(|item| {
        ...
        match &mut dispatcher {
            RequestDispatcher {
                req: Some(req),
                server_state: this,
            } if this.event_loop_state.shutdown_requested => {
