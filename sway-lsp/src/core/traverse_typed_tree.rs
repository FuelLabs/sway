...
fn handle_declaration(declaration: &TypedDeclaration, tokens: &mut TokenMap) {
    match declaration {
        ...
        TypedDeclaration::EnumDeclaration(enum_decl) => {
            if let Some(token) = tokens.get_mut(&to_ident_key(&enum_decl.name)) {
                token.typed = Some(TypedAstToken::TypedDeclaration(declaration.clone()));
            }

            for variant in &enum_decl.variants {
                if let Some(token) = tokens.get_mut(&to_ident_key(&variant.name)) {
        ...
        TypedDeclaration::Reassignment(reassignment) => {
            ...
            if let Some(token) = tokens.get_mut(&to_ident_key(&reassignment.lhs_base_name)) {
        ...
        TypedDeclaration::ImplTrait(TypedImplTrait {
            ...
            for ident in &trait_name.prefixes {
                if let Some(token) = tokens.get_mut(&to_ident_key(ident)) {
                    token.typed = Some(TypedAstToken::TypedDeclaration(declaration.clone()));
                }
            }

            if let Some(token) = tokens.get_mut(&to_ident_key(&trait_name.suffix)) {
                token.typed = Some(TypedAstToken::TypedDeclaration(declaration.clone()));
            }

            for method in methods {
                if let Some(token) = tokens.get_mut(&to_ident_key(&method.name)) {
                ...
                if let Some(token) = tokens.get_mut(&to_ident_key(&return_type_ident)) {
                    token.typed = Some(TypedAstToken::TypedFunctionDeclaration(method.clone()));
                }
            }
        }
        TypedDeclaration::AbiDeclaration(abi_decl) => {
            if let Some(token) = tokens.get_mut(&to_ident_key(&abi_decl.name)) {
                token.typed = Some(TypedAstToken::TypedDeclaration(declaration.clone()));
            }

            for trait_fn in &abi_decl.interface_surface {
                if let Some(token) = tokens.get_mut(&to_ident_key(&trait_fn.name)) {
                    token.typed = Some(TypedAstToken::TypedTraitFn(trait_fn.clone()));
                }
            }
        }
        TypedDeclaration::GenericTypeForFunctionScope { name, .. } => {
            if let Some(token) = tokens.get_mut(&to_ident_key(name)) {
                token.typed = Some(TypedAstToken::TypedDeclaration(declaration.clone()));
            }
        }
...
fn handle_expression(expression: &TypedExpression, tokens: &mut TokenMap) {
    match &expression.expression {
        ...
        TypedExpressionVariant::FunctionApplication {
            ...
            if let Some(token) = tokens.get_mut(&to_ident_key(&call_path.suffix)) {
                token.typed = Some(TypedAstToken::TypedExpression(expression.clone()));
            }
        ...
        TypedExpressionVariant::StructExpression {
            ...
            if let Some(token) = tokens.get_mut(&to_ident_key(struct_name)) {
                token.typed = Some(TypedAstToken::TypedExpression(expression.clone()));
            }
        ...
        TypedExpressionVariant::StructFieldAccess {
            ...
            if let Some(token) = tokens.get_mut(&to_ident_key(&field_to_access.name)) {
                token.typed = Some(TypedAstToken::TypedExpression(expression.clone()));
            }
        }
        ...
        TypedExpressionVariant::AbiCast {
            ...
            if let Some(token) = tokens.get_mut(&to_ident_key(&abi_name.suffix)) {
                token.typed = Some(TypedAstToken::TypedExpression(expression.clone()));
            }
