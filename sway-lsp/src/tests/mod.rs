...
mod tests {
    use crate::{
        ...
        tests::{
            lsp,
            utils::{
                assert_server_requests, dir_contains_forc_manifest, doc_comments_dir,
                e2e_language_dir, e2e_test_dir, get_fixture, runnables_test_dir,
    ...
    async fn write_all_example_asts() {
        ...
        for entry in entries {
            ...
            if manifest_dir.is_dir() {
                ...
                let _ =
                    lsp::show_ast_request(&mut service, &uri, "lexed", example_dir.clone()).await;
                let _ =
                    lsp::show_ast_request(&mut service, &uri, "parsed", example_dir.clone()).await;
