...
///
/// ```
/// use sway_lsp::utils::semaphore::Semaphore;
///
...
impl Semaphore {
    ...
    ///
    /// If a permit is available, this function will immediately decrease the
    /// count of available permits and return `Some(())`.
    ///
    /// If no permits are currently available, this function will not block
    /// and will immediately return `None`.
    ///
    /// This function is similar to `acquire`, but will not block waiting
    /// for a permit to become available.
    ///
    /// # Returns
    ///
    /// - `Some(())` if a permit was successfully acquired.
    /// - `None` if no permits are currently available.
