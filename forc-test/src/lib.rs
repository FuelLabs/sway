...
impl<'a> PackageTests {
    ...
    pub(crate) fn run_tests(&self, print_receipts: bool) -> anyhow::Result<TestedPackage> {
        ...
        let tests = pkg_with_tests
            ...
            .map(|entry| {
                ...
                let (state, duration) = exec_test(&pkg_with_tests.bytecode, offset, test_setup, print_receipts);
...
impl BuiltTests {
    ...
    pub fn run(self, print_receipts: bool) -> anyhow::Result<Tested> {
        run_tests(self, print_receipts)
    }
}
...
fn run_tests(built: BuiltTests, print_receipts: bool) -> anyhow::Result<Tested> {
    match built {
        BuiltTests::Package(pkg) => {
            let tested_pkg = pkg.run_tests(print_receipts)?;
            Ok(Tested::Package(Box::new(tested_pkg)))
        }
        BuiltTests::Workspace(workspace) => {
            let tested_pkgs = workspace
                .into_iter()
                .map(|pkg| pkg.run_tests(print_receipts))
