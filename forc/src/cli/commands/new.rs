...
use anyhow::anyhow;
use clap::Parser;
use forc_util::{forc_result_bail, validate_project_and_package_name, ForcResult};
...
pub(crate) fn exec(command: Command) -> ForcResult<()> {
    ...
    match &name {
        Some(name) => validate_project_and_package_name(name, "project name")?,
        None => {
            ...
            let directory_name = project_path
                ...
                .to_string_lossy();
            validate_project_and_package_name(&directory_name, "project_name")?;
        }
    }
