...
fn ra(instruction: Instruction) -> Option<String> {
    reg_id_to_string(match instruction {
        ...
        Instruction::SCLR(op) => Some(op.ra()),
        Instruction::SRW(op) => Some(op.ra()),
        Instruction::SRWQ(op) => Some(op.ra()),
        Instruction::SRDD(op) => Some(op.ra()),
        Instruction::SRDI(op) => Some(op.ra()),
        Instruction::SWW(op) => Some(op.ra()),
        Instruction::SWWQ(op) => Some(op.ra()),
        Instruction::SWRD(op) => Some(op.ra()),
        Instruction::SWRI(op) => Some(op.ra()),
        Instruction::SUPD(op) => Some(op.ra()),
        Instruction::SUPI(op) => Some(op.ra()),
        Instruction::SPLD(op) => Some(op.ra()),
...
fn rb(instruction: Instruction) -> Option<String> {
    reg_id_to_string(match instruction {
        ...
        Instruction::SCLR(op) => Some(op.rb()),
        Instruction::SRW(op) => Some(op.rb()),
        Instruction::SRWQ(op) => Some(op.rb()),
        Instruction::SRDD(op) => Some(op.rb()),
        Instruction::SRDI(op) => Some(op.rb()),
        Instruction::SWW(op) => Some(op.rb()),
        Instruction::SWWQ(op) => Some(op.rb()),
        Instruction::SWRD(op) => Some(op.rb()),
        Instruction::SWRI(op) => Some(op.rb()),
        Instruction::SUPD(op) => Some(op.rb()),
        Instruction::SUPI(op) => Some(op.rb()),
        Instruction::SPLD(op) => Some(op.rb()),
...
fn rc(instruction: Instruction) -> Option<String> {
    reg_id_to_string(match instruction {
        ...
        Instruction::SRDD(op) => Some(op.rc()),
        Instruction::SRDI(op) => Some(op.rc()),
        Instruction::SWW(op) => Some(op.rc()),
        Instruction::SWWQ(op) => Some(op.rc()),
        Instruction::SWRD(op) => Some(op.rc()),
        Instruction::SUPD(op) => Some(op.rc()),
        Instruction::SUPI(op) => Some(op.rc()),
...
fn rd(instruction: Instruction) -> Option<String> {
    reg_id_to_string(match instruction {
        ...
        Instruction::SRDD(op) => Some(op.rd()),
        Instruction::SUPD(op) => Some(op.rd()),
...
fn imm(instruction: Instruction) -> Option<String> {
    match instruction {
        ...
        Instruction::SRDI(op) => imm06_to_string(op.imm06()),
        Instruction::SUPI(op) => imm06_to_string(op.imm06()),
        Instruction::SWRI(op) => imm12_to_string(op.imm12()),
        _ => None,
    }
}
