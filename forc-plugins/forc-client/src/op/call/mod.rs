...
mod tests {
    ...
    //     let function_signature = "fn f(a: (), b: u64, c: u128) -> [bool] {}";
    //     // let function_signature = "struct Container<T> { value: T, description: str[10] };fn generic_function(a: Container<u32>) -> u32 {}";
    //     let item = with_handler(|handler| {
    //         let token_stream = sway_parse::lex(
    //             handler,
    ...
    //     let ItemKind::Fn(item_fn) = item.value else {
    //         panic!("Expected a function");
    //     };

    //     // assert_eq!(item_fn.fn_signature.name.as_str(), "f");

    //     let FnArgs::Static(args) = item_fn.fn_signature.arguments.inner else {
    //         panic!("Expected a static function");
    //     };

    //     println!("{:?}\n\n", args);

    //     args.value_separator_pairs
    //         .iter()
    //         .map(|(arg, _)| &arg.ty)
    //         .chain(args.final_value_opt.iter().map(|arg| &arg.ty))
    //         .for_each(|ty| {
    //             // println!("arg: {:?}", ty);
    //             // if let Ty::Path(path) = ty {
    //             //     println!("arg: {:?}", path.prefix.name);
    //             // } else {
    //             //     panic!("Expected a path, got {:?}", ty);
    //             // }
    //             match ty {
    //                 Ty::Path(path) => {
    ...
    //                 }
    //                 Ty::Tuple(tuple) => {
    //                     let values = tuple.inner.clone().to_tys();
    //                     if values.is_empty() {
    //                         // unit type token
    //                     } else {
    ...
    //                     }
    //                 }
    //                 _ => panic!("Expected a path, got {:?}", ty),
    //             }
    //         });

    //     let (_, ty) = item_fn.fn_signature.return_type_opt.unwrap();
    //     println!("return: {:?}", ty);

    //     // println!("{:?}", item_fn.fn_signature);

    //     assert!(false);
    // }

    // #[test]
    // fn parse_function_multiple_complex() {
    //     let item = parse::<Item>(
    //         r#"
    //         trait Convert<T> {
    //             fn from(t: T) -> Self;
    ...
    //         fn simple_function(a: u32, b: u64) -> u64;

    //         Function using string
    //         fn string_function(input: str) -> str;

    //         Function using string
    //         fn string_array_function(input: str[10]) -> str[10];

    //         Function using string slice
    //         fn string_slice_function(input: []str) -> []str;

    //         Function using string vector
    //         fn string_vector_function(input: Vec<str>) -> Vec<str>;

    //         Function using generics
    //         fn generic_function(container: Container<u32>) -> u32;

    //         Function using Option
    //         fn option_function(value: Option<u64>) -> u64;

    //         Function using struct
    //         fn struct_function(user: Identity) -> u64;

    //         Function using enum
    //         fn enum_function(status: Status) -> bool;

    //         Function using vector
    //         fn vector_function(numbers: Vec<u64>) -> u64;

    //         Function using slice
    //         fn slice_function(numbers: [u64]) -> u64;

    //         Function using tuple
    //         fn tuple_function(data: (u64, bool)) -> bool;

    //         Function converting struct to another struct
    //         fn convert_struct(square: Square) -> Rectangle;
    //         "#,
    //     );
    ...
    //     // args.value_separator_pairs.iter().for_each(|(arg, _)| {
    //     //     println!("arg: {:?}", arg.ty);
    //     // });

    //     // // optional final value
    //     // args.final_value_opt.map(|arg| {
    //     //     println!("final arg: {:?}", arg.ty);
    //     // });
