...
// type FnKey = (Span, FnParams, FnTypeParams, u64);
type FnKey = u64;
...
impl CompiledFunctionCache {
    ...
    fn ty_function_decl_to_unique_function(
        ...
        // println!("Not parts of the key");

        // let mut hasher = DefaultHasher::default();
        // std::hash::Hash::hash(&fn_key, &mut hasher);
        // println!("    Key: {}", hasher.finish());

        // let mut hasher = DefaultHasher::default();
        // decl.hash(&mut hasher, engines);
        // println!("    Body hash: {}", hasher.finish());

        // println!("Parts of the key");
        // if let Some(fn_key) = fn_key.as_ref() {
        //     println!("    Span: {:?}", fn_key.0);

        //     println!("    Parameters:");
        //     for i in &fn_key.1 {
        //         println!("        {:?}", engines.help_out(i));
        //     }
