...
use crate::semantic_analysis::{
    ast_node::{TypedExpression, TypedStorageDeclaration, TypedVariableDeclaration},
...
use super::declaration::{CreateTypeId, Monomorphize, MonomorphizeHelper};
...
impl Root {
    ...
    pub(crate) fn find_method_for_type(
            ...
            .find(|TypedFunctionDeclaration { name, .. }| name == method_name)
        {
            ...
            None => {
                if arguments.get(0).map(|x| look_up_type_id(x.return_type))
...
impl Namespace {
    ...
    pub fn find_module_path(&self, prefixes: &[Ident]) -> PathBuf {
