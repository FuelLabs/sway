...
fn type_check_size_of_type(
    ...
    let initial_type_id = type_engine.insert_type(decl_engine, initial_type_info);
    let type_id = check!(
...
fn type_check_is_reference_type(
    ...
    ok(
        (
            intrinsic_function,
            type_engine.insert_type(decl_engine, TypeInfo::Boolean),
        ),
        warnings,
        errors,
    )
...
fn type_check_get_storage_key(
    ...
    ok(
        (
            ...
            type_engine.insert_type(decl_engine, TypeInfo::B256),
        ),
        vec![],
        vec![],
    )
...
fn type_check_addr_of(
    ...
    let return_type = type_engine.insert_type(decl_engine, TypeInfo::RawUntypedPtr);
    ok((intrinsic_function, return_type), warnings, errors)
}
...
fn type_check_state_store_word(
    ...
    let mut ctx = ctx.with_type_annotation(type_engine.insert_type(decl_engine, TypeInfo::Unknown));
    let val_exp = check!(
        ty::TyExpression::type_check(ctx.by_ref(), arguments[1].clone()),
        return err(warnings, errors),
        warnings,
    ...
    let type_argument = type_arguments.get(0).map(|targ| {
        let mut ctx =
            ctx.with_type_annotation(type_engine.insert_type(decl_engine, TypeInfo::Unknown));
        let initial_type_info = check!(
            CompileResult::from(
                type_engine
                    .to_typeinfo(targ.type_id, &targ.span)
    ...
    let return_type = type_engine.insert_type(decl_engine, TypeInfo::Tuple(vec![]));
    ok((intrinsic_function, return_type), warnings, errors)
}
...
fn type_check_binary_op(
    ...
    ok(
        (
            ...
            type_engine.insert_type(decl_engine, arg_ty),
        ),
        warnings,
        errors,
    )
...
fn type_check_revert(
    ...
    ok(
        (
            ...
            type_engine.insert_type(decl_engine, TypeInfo::Unknown), // TODO: change this to the `Never` type when
                                                                     // available
        ),
        warnings,
        errors,
...
fn type_check_ptr_ops(
    ...
    ok(
        (
            ...
            type_engine.insert_type(decl_engine, lhs_ty),
        ),
        warnings,
        errors,
    )
