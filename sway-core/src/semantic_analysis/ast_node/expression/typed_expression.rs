...
impl ty::TyExpression {
    ...
    fn type_check_storage_access(
        ...
        if !ctx.namespace.module().root_items().has_storage_declared() {
        ...
        let (storage_access, mut access_type) =
            ctx.namespace.module().root_items().apply_storage_load(
                handler,
                ctx.engines,
                ctx.namespace,
                checkee,
        ...
        match lhs {
            ReassignmentTarget::VariableExpression(var) => {
                ...
                let (ty_of_field, _ty_of_parent) =
                    ctx.namespace.module().root_items().find_subfield_type(
                        handler,
                        ctx.engines(),
                        ctx.namespace,
                        &base_name,
