...
pub(crate) fn instantiate_function_application(
    ...
    check!(
        check_function_arguments_arity(arguments.len(), &(&function_decl).into(), &call_path),
    ...
    let exp = ty::TyExpression {
        expression: ty::TyExpressionVariant::FunctionApplication {
            ...
            function_decl_id: de_insert_function(function_decl),
            self_state_idx: None,
            selector: None,
        },
        return_type,
...
pub(crate) fn check_function_arguments_arity(
    arguments_len: usize,
    func_signature: &ty::TyFunctionSignature,
    call_path: &CallPath,
) -> CompileResult<()> {
    let warnings = vec![];
    let mut errors = vec![];
    match arguments_len.cmp(&func_signature.parameters.len()) {
        std::cmp::Ordering::Equal => ok((), warnings, errors),
        std::cmp::Ordering::Less => {
            errors.push(CompileError::TooFewArgumentsForFunction {
                span: call_path.span(),
                method_name: func_signature.name.clone(),
