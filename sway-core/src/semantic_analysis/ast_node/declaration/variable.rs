...
impl ty::TyVariableDecl {
	...
	let problem = var_decl.name.as_str() == "__matched_value_1" &&
	    current_mod_path.len() == 1 && current_mod_path[0].as_str() == "diverging_exprs";

	if problem {
	    dbg!(type_ascription.type_id);
	    dbg!(&*type_engine.get(type_ascription.type_id));
	}
	...
	if problem {
//	    dbg!(&body);
//	    dbg!(&*type_engine.get(body.return_type));
	    dbg!(type_ascription.type_id);
	    dbg!(&*type_engine.get(type_ascription.type_id));
	}
...
//	}
	...
	typechecking of the body changes what type_ascription.type_id refers to - it changes from Unknown to Boolean.
	    This is almost certainly to do with the unification stuff that happens at the end of ast_node/code_block.rs::type_check.
	    An obvious guess is that the type of the code block is determined as the type of the implicit return rather than as (Never if a diverging expression exists, or else the implicit return). See TyCodeBloc::compute_return_type_and_span.
...
//	dbg!(&*type_engine.get(body.return_type));
	...
	//       in a way to always use the context provided by the LHS, and use the unified type of
	//       LHS and RHS as the return type of the RHS.  Remove this special case as a part of
	//       the initiative of improving type inference.
        let return_type =
...
//	dbg!(&*type_engine.get(return_type));
