    ...
    'a: 'b,
{
        ...
        T: Parse,
    {
        ...
        let source = sway_types::span::Source::from(src);
        let source = if let Some(id) = &autogenerated_source_id {
            // Register so that metadata printing can inline the autogenerated source code.
            engines.se().get_or_create_source_buffer(id, source)
        } else {
            source
        };

        let token_stream =
            sway_parse::lex(&handler, source, 0, src.len(), autogenerated_source_id).unwrap();
