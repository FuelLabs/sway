...
use crate::{
    ...
    language::{
        parsed::{
            ...
            EnumDeclaration, EnumVariant, Expression, ExpressionKind, FunctionDeclaration,
            FunctionParameter, ImplItem, ImplSelf, ImplTrait, ParseModule, ParseProgram,
            ReassignmentTarget, Scrutinee, StorageDeclaration, StructDeclaration,
            StructExpressionField, StructField, StructScrutineeField, Supertrait, TraitDeclaration,
            TraitFn, TraitItem, TraitTypeDeclaration, TypeAliasDeclaration, VariableDeclaration,
        },
...
impl ResolveSymbols for ParseProgram {
    fn resolve_symbols(&mut self, handler: &Handler, mut ctx: SymbolResolveContext) {
        ...
        ctx.namespace_mut().root.resolve_options.resolve_typed = false;
        root.resolve_symbols(handler, ctx.by_ref());
        ctx.namespace_mut().root.resolve_options.resolve_typed = true;
    }
}
...
impl ResolveSymbols for Declaration {
    fn resolve_symbols(&mut self, handler: &Handler, ctx: SymbolResolveContext) {
        match self {
            ...
            Declaration::ImplTrait(decl_id) => decl_id.resolve_symbols(handler, ctx),
            Declaration::ImplSelf(decl_id) => decl_id.resolve_symbols(handler, ctx),
...
impl ResolveSymbols for ParsedDeclId<ImplTrait> {
    fn resolve_symbols(&mut self, handler: &Handler, ctx: SymbolResolveContext) {
        let pe = ctx.engines().pe();
        let mut impl_trait = ctx.engines().pe().get_impl_trait(self).as_ref().clone();
        impl_trait.resolve_symbols(handler, ctx);
        pe.replace(*self, impl_trait);
    }
}

impl ResolveSymbols for ParsedDeclId<ImplSelf> {
    fn resolve_symbols(&mut self, handler: &Handler, ctx: SymbolResolveContext) {
        let pe = ctx.engines().pe();
        let mut impl_decl = ctx.engines().pe().get_impl_self(self).as_ref().clone();
        impl_decl.resolve_symbols(handler, ctx);
        pe.replace(*self, impl_decl);
    }
}
...
impl ResolveSymbols for ImplTrait {
    fn resolve_symbols(&mut self, handler: &Handler, mut ctx: SymbolResolveContext) {
...
impl ResolveSymbols for ImplSelf {
    fn resolve_symbols(&mut self, handler: &Handler, mut ctx: SymbolResolveContext) {
        self.impl_type_parameters
            .iter_mut()
            .for_each(|f| f.resolve_symbols(handler, ctx.by_ref()));
    ...
    fn resolve_symbols(&mut self, handler: &Handler, mut ctx: SymbolResolveContext) {
        match self {
            ...
            ExpressionKind::FunctionApplication(expr) => {
                //expr.call_path_binding.resolve_symbol(handler, ctx.by_ref());
