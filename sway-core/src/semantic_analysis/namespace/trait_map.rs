...
impl TraitMap {
    ...
    //      There is at least one type parameter Pi which, taking fundamental types into account,
    //          is local to the current crate
    //      Within the type Pi, all type parameters are covered by Pi
    // All types Pj such that j < i do not contain T0...Tn at any level of depth (i.e. the types 
    // are fully visible, — “visible” meaning that the type is a known type and not a type
