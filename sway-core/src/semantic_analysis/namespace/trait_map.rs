...
use crate::{
    ...
    ReplaceSelfType, TraitConstraint, TypeArgument, TypeEngine, TypeInfo, TypeSubstMap, UnifyCheck,
};
...
impl TraitMap {
    ...
    pub(crate) fn get_methods_for_type(
        ...
        for entry in self.trait_impls.iter() {
            if type_engine
                .get(type_id)
                .eq(&type_engine.get(entry.key.type_id), engines)
            {
    ...
    pub(crate) fn get_methods_for_type_and_trait_name(
        ...
        for e in self.trait_impls.iter() {
            ...
            if &map_trait_name == trait_name
                && type_engine
                    .get(type_id)
                    .eq(&type_engine.get(e.key.type_id), engines)
            {
    ...
    pub(crate) fn check_if_trait_constraints_are_satisfied_for_type(
        ...
        let all_impld_traits: BTreeMap<Ident, TypeId> = self
            ...
            .filter_map(|e| {
                ...
                if type_engine
                    .get(type_id)
                    .eq(&type_engine.get(key.type_id), engines)
                {
