...
fn expr_func_app_to_expression_kind(
    ...
    match Intrinsic::try_from_str(call_seg.name.as_str()) {
        ...
        Some(Intrinsic::Dbg) if last.is_none() && !is_relative_to_root => {
            ...
            let f_decl_pid = engines.pe().insert(
                VariableDeclaration {
                    name: f_ident.clone(),
            ...
            let current_file = span.source_id().map(|x| engines.se().get_path(x)).unwrap_or_default();
            let current_line_col = span.line_col();

            // find the manifest path of the current span
            let program_id = engines.se().get_program_id_from_manifest_path(&current_file).unwrap();
            let manifest_path = engines.se().get_manifest_path_from_program_id(&program_id).unwrap();            
            let current_file = current_file.display().to_string().replace(&manifest_path.display().to_string(), "");
                                    ...
                                    arguments: vec![
                                        ...
                                        Expression { 
                                            kind: ExpressionKind::Literal(
                                                Literal::String(Span::from_string(s.to_string()))
                                            ),
                                            span: span.clone()
                                        }
                                    ]
            ...
            let block = CodeBlock {
                contents: vec![
                    ...
                    statement_let_to_ast_nodes_unfold(
                        context, handler, engines, Pattern::AmbiguousSingleIdent(arg_ident.clone()), None, arguments[0].clone(), span.clone()
                    ).unwrap().pop().unwrap(),
                    // let mut f = Formatter { };
                    AstNode {
                        content: AstNodeContent::Declaration(Declaration::VariableDeclaration(f_decl_pid)),
                        span: span.clone(),
                    },                    
                    // f.print_str("[" + <current file> + ":" + <current line> + ":" + <current col> + "] = ");
                    ast_node_to_print_str(
                        f_ident.clone(),
                        &format!("[{}:{}:{}] = ", current_file, current_line_col.start.line, current_line_col.start.col), 
                        &span
                    ),
                    // arg.fmt(f);
                    AstNode {
                    ...
                    AstNode {
                        content: AstNodeContent::Expression(
                            Expression {
                                kind: ExpressionKind::ImplicitReturn(
                                    Box::new(
                                        Expression {
                                        kind: ExpressionKind::AmbiguousVariableExpression(arg_ident.clone()),
                                        span: span.clone(),
                                        }
                                    )
                                ),
