...
pub(crate) struct Solver<'a> {
    ...
    fn_graph: RwLock<Graph<DeclId<TyFunctionDecl>, Option<SubstList>>>,

    /// Graph representing the flow of monomorphization for structs.
    struct_graph: RwLock<Graph<DeclId<TyStructDecl>, Option<SubstList>>>,

    /// Graph representing the flow of monomorphization for enums.
    enum_graph: RwLock<Graph<DeclId<TyEnumDecl>, Option<SubstList>>>,

    /// Graph representing the flow of monomorphization for traits.
    trait_graph: RwLock<Graph<DeclId<TyTraitDecl>, Option<SubstList>>>,
}
...
impl<'a> Solver<'a> {
    ...
    pub(crate) fn new(engines: Engines<'a>) -> Solver<'a> {
        ...
        Solver {
            ...
            decl_engine,
            instructions: vec![],
            fn_graph: RwLock::new(Graph::new()),
            struct_graph: RwLock::new(Graph::new()),
            enum_graph: RwLock::new(Graph::new()),
            trait_graph: RwLock::new(Graph::new()),
        }
    }
