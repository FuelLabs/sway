...
pub(crate) fn gather_from_ty(ctx: GatherContext, type_id: TypeId) {
    use TypeInfo::*;
    ...
    match type_engine.get(type_id) {
        ...
        Tuple(elems) => {
            for elem in elems {
                gather_from_ty(ctx, elem.type_id);
            }
        }
        ContractCaller {
            abi_name: _,
        ...
        Custom {
            ...
            if let Some(type_args) = type_arguments {
                for type_arg in type_args {
                    gather_from_ty(ctx, type_arg.type_id);
                }
            }
        }
        Array(ty, _) => {
            gather_from_ty(ctx, ty.type_id);
        }
        Storage { fields } => {
            for field in fields {
...
pub(crate) fn gather_from_trait_constraints(ctx: GatherContext, constraints: Vec<TraitConstraint>) {
