...
/// The optional portion of the prelude, in bytes: the `LW $cs` / `ADD $cs` pair that
/// initializes the configurable-section register. This is emitted only when the
...
///
/// The prelude size is **dynamic**: the two instructions that initialize the
...
/// is therefore 40 bytes in that case and 32 bytes otherwise. The fixed metadata
/// positions (`DATA_START` at byte 8, `CONFIGURABLES_OFFSET` at byte 16) do not
/// move regardless.
...
/// (`$locals_base`). External tooling that inspects generated bytecode and needs to
/// locate the start of a function's prologue (e.g. `forc test` scanning for the first
/// `MOVE $locals_base $sp`) must use this constant so it stays in sync with the
/// compiler's register reservation.
pub const LOCALS_BASE_REGISTER: u8 = asm_generation::fuel::compiler_constants::LOCALS_BASE;
