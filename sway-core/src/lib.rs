    ...
    if build_config.build_target == BuildTarget::Fuel {
        ...
        match build_config.optimization_level {
            OptLevel::Opt1 => {
                ...
                // Second cleanup round. With the #6065 escape-analysis fix, the generated
                // `__entry`/decode code is now largely escape-Complete, so memcpyopt/SROA/
                // mem2reg/CSE can collapse its copy chains and scalarize decode temporaries
                // that the first round (and pre-#6065 builds) had to leave alone.
