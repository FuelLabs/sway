...
pub fn compile_to_ast(
    ...
    // Check that all storage initializers can be evaluated at compile time. The actual slots
    // generated are not used here as they don't ha
    let CompileResult {
        value: typed_program_with_storage_slots_result,
        warnings: new_warnings,
        errors: new_errors,
    } = typed_program.get_initialized_storage_slots();
    warnings.extend(new_warnings);
    errors.extend(new_errors);
