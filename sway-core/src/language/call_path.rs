    ...
    pub fn to_fullpath(&self, engines: &Engines, namespace: &Namespace) -> CallPath {
        ...
        if self.prefixes.is_empty() {
            ...
            if let Some(use_synonym) = namespace.module_id(engines).read(engines, |m| {
                ...
                is_absolute = true;
                let submodule = namespace.module_id(engines).read(engines, |m| {
                    m.submodule(engines, &[use_synonym.0[0].clone()])
                });
        ...
        } else if let Some(m) = namespace.module_id(engines).read(engines, |m| {
            m.submodule(engines, &[self.prefixes[0].clone()])
        }) {
