...
fn to_bytecode_mut(
    ...
    fn op_size_in_bytes(data_section: &DataSection, item: &AllocatedOp) -> u64 {
        match &item.opcode {
            AllocatedInstruction::LoadFromDataSection(_reg, data_label)
                if !data_section
            ...
            AllocatedInstruction::AddrFromDataSection(_, _data_id) => 8,
            AllocatedInstruction::ConfigurablesOffsetPlaceholder => 8,
            AllocatedInstruction::DataSectionOffsetPlaceholder => 8,
            // A literal-pool address lowers to a single `ADDI dest, $pc, delta`
            // (1 instruction, 4 bytes), which is *not* size-neutral with
            // `AddrFromDataSection` (`MOVI` + `ADD`, 8 bytes). The
            // `move_to_literal_pools` pass runs before label resolution, so the
            // shrunk size is accounted for when computing offsets and far jumps.
            AllocatedInstruction::AddrFromLiteralPool(_, _) => 4,
            // A value load from a pool lowers to `MOVI; ADD $pc; LW` (3
            // instructions). Not currently generated by `move_to_literal_pools`.
            AllocatedInstruction::LoadFromLiteralPool(_, _) => 12,
            // A literal pool emits its entries' bytes inline as raw data.
            AllocatedInstruction::LiteralPool(entries) => {
                DataSection::literal_pool_size_bytes(entries)
            }
    ...
    // Record the offset (from instruction start) of every op, used to resolve
    // literal-pool references below.
    let mut op_offsets: Vec<u64> = Vec::with_capacity(ops.len());
    for op in ops.iter() {
        op_offsets.push(offset_from_instr_start);
        match &op.opcode {
            AllocatedInstruction::LoadFromDataSection(_reg, data_label)
                if !data_section
                    .has_copy_type(data_label)
    ...
    // Resolve literal-pool references. Each function ends with a single
    // `LiteralPool` op carrying its entries; `AddrFromLiteralPool`/
    // `LoadFromLiteralPool` ops occur earlier in the same function and refer to
    // an entry in that pool. For each such reference, compute the absolute
    // (from instruction-start) byte offset of its target entry, so the emit
    // loop can lower it `$pc`-relatively.
    let pool_op_indices: Vec<usize> = ops
        .iter()
        .enumerate()
        .filter_map(|(i, op)| match &op.opcode {
            AllocatedInstruction::LiteralPool(_) => Some(i),
            _ => None,
        })
        .collect();

    /// The resolved target of a literal-pool reference, in bytes from
    /// instruction start.
    enum PoolTarget {
        Addr { target_abs: u64 },
        Load { target_abs: u64, is_byte: bool },
    }

    let pool_targets: Vec<Option<PoolTarget>> = ops
        .iter()
        .enumerate()
        .map(|(i, op)| match &op.opcode {
            AllocatedInstruction::AddrFromLiteralPool(_, id) => {
                let pidx = *pool_op_indices
                    .iter()
                    .find(|&&p| p > i)
                    .expect("literal pool reference has no following LiteralPool");
                let AllocatedInstruction::LiteralPool(entries) = &ops[pidx].opcode else {
                    unreachable!();
                };
                let pool_base = op_offsets[pidx];
                let target_abs =
                    pool_base + DataSection::literal_pool_entry_offset(entries, id.0 as usize) as u64;
                Some(PoolTarget::Addr { target_abs })
            }
            AllocatedInstruction::LoadFromLiteralPool(_, id) => {
                let pidx = *pool_op_indices
                    .iter()
                    .find(|&&p| p > i)
                    .expect("literal pool reference has no following LiteralPool");
                ...
                let target_abs =
                    pool_base + DataSection::literal_pool_entry_offset(entries, id.0 as usize) as u64;
                Some(PoolTarget::Load {
                    target_abs,
                    is_byte: entries[id.0 as usize].is_byte(),
                })
            }
            _ => None,
        })
    ...
    for (op_ix, op) in ops.iter().enumerate() {
        ...
        let fuel_op = match &op.opcode {
            // Literal-pool references are resolved here, where the full op
            // stream and per-op offsets are known. They lower to a single
            // `$pc`-relative `ADDI` (see `allocated_ops::addr_from_literal_pool`).
            AllocatedInstruction::AddrFromLiteralPool(dest, _) => {
                let PoolTarget::Addr { target_abs } =
                    pool_targets[op_ix].as_ref().expect("resolved pool target")
                else {
                    unreachable!();
                };
                FuelAsmData::Instructions(AllocatedInstruction::addr_from_literal_pool(
                    dest,
                    *target_abs,
                    offset_from_instr_start,
                ))
            }
            AllocatedInstruction::LoadFromLiteralPool(dest, _) => {
                let PoolTarget::Load { target_abs, is_byte } =
                    pool_targets[op_ix].as_ref().expect("resolved pool target")
                else {
                    unreachable!();
                };
                FuelAsmData::Instructions(AllocatedInstruction::load_from_literal_pool(
                    dest,
                    *target_abs,
                    offset_from_instr_start,
                    *is_byte,
        ...
        match fuel_op {
            ...
            FuelAsmData::LiteralPoolBytes(bytes) => {
                if build_config.print_bytecode {
                    print!("{}{:#010x} ", " ".repeat(indentation), bytecode.len());
                    println!(";; literal pool, {} bytes", bytes.len());
                }

                // Literal-pool bytes are always word-aligned (a multiple of
                // 4), so they advance `half_word_ix` cleanly and keep the
                // subsequent data section word-aligned.
                debug_assert_eq!(bytes.len() % 4, 0, "literal pool must be 4-byte aligned");
                bytecode.extend(bytes.iter());
                half_word_ix += bytes.len() / 4;
            }
    ...
    if build_config.print_bytecode {
        ...
        // Only dump entries that are actually serialized; relocated
        // non-configurables now live in a function's literal pool (dumped
        // inline where the `LiteralPool` op appears), not in the data section.
        for (i, entry) in data_section.iter_serialized_entries() {
