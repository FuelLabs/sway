...
impl<'ir, 'eng> FuelAsmBuilder<'ir, 'eng> {
    ...
    fn compile_get_config(
        ...
        if let Some(g) = self.globals_section.get_by_name(name) {
            ...
            // configurable. Address it relative to `$ds` via `AddrDataId` (near
            // `ADDI` when Imm12 fits, otherwise far `MOVI`+`ADD`).
            let span = self
                .md_mgr
            ...
            let dataid = self
                ...
                .clone();
            self.cur_bytecode.push(Op {
                opcode: Either::Left(VirtualOp::AddrDataId(addr_reg.clone(), dataid)),
                comment: format!("get address of configurable {name}"),
                owning_span: Some(span),
            });
            self.reg_map.insert(*addr_val, addr_reg);
