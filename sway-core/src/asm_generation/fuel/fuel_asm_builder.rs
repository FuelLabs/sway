        ...
        if let Some(g) = self.globals_section.get_by_name(name) {
            self.cur_bytecode.push(Op {
                opcode: either::Either::Left(VirtualOp::ADDI(
                    addr_reg.clone(),
                    VirtualRegister::Constant(ConstantRegister::StackStartPointer),
                    VirtualImmediate12::new(g.offset_in_bytes),
                )),
...
<<<<<<< HEAD
            // configurable.
            let dataid = self.configurable_to_data_id.get(name).ok_or_else(|| {
                CompileError::InternalOwned(
                    format!("configurable '{name}' missing from data section map"),
                    self.md_mgr
...
>>>>>>> de3082938 (new register pointing to the configurable section to allow access with just one addi)
            self.reg_map.insert(*addr_val, addr_reg);
        }
    ...
    fn compile_gtf(
        ...
        self.cur_bytecode.push(Op {
            opcode: either::Either::Left(VirtualOp::GTF(
                instr_reg.clone(),
                index_reg,
                VirtualImmediate12::new(tx_field_id),
            )),
    ...
    fn compile_mem_copy_bytes(
        ...
        match VirtualImmediate12::try_new(byte_len, Span::dummy()) {
            Ok(byte_len) => {
            ...
            Err(_) => {
                match VirtualImmediate18::try_new(byte_len, Span::dummy()) {
                    Ok(byte_len) => {
    ...
    fn compile_mem_clear_val(
        ...
        match VirtualImmediate18::try_new(
            len_in_bytes,
            owning_span.clone().unwrap_or(Span::dummy()),
        ) {
            Ok(len) => {
                self.cur_bytecode.push(Op {
    ...
    fn compile_jmp_mem(&mut self, instr_val: &Value) -> Result<(), CompileError> {
        ...
        self.cur_bytecode.push(Op {
            ...
            opcode: Either::Left(VirtualOp::LW(
                target_reg.clone(),
                VirtualRegister::Constant(ConstantRegister::HeapPointer),
                VirtualImmediate12::try_new(0, Span::dummy()).unwrap(),
            )),
        ...
        self.cur_bytecode.push(Op {
            ...
            opcode: Either::Left(VirtualOp::DIVI(
                by4_reg.clone(),
                is_target_reg.clone(),
                VirtualImmediate12::try_new(4, Span::dummy()).unwrap(),
            )),
    ...
    pub(super) fn immediate_to_reg<S: Into<String>>(
        ...
        if imm <= compiler_constants::TWELVE_BITS && base.is_some() {
            ...
            self.cur_bytecode.push(Op {
                opcode: Either::Left(VirtualOp::MOVI(reg.clone(), VirtualImmediate18::new(imm))),
