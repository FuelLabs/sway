...
impl AllocatedAbstractInstructionSet {
    ...
    pub(crate) fn worst_case_instruction_size(op: &AllocatedAbstractOp) -> u64 {
        use ControlFlowOp::*;
        match op.opcode {
            ...
            Either::Left(
                AllocatedInstruction::LoadFromDataSection(_, _) | AllocatedInstruction::AddrFromDataSection(_, _),
            ) => 2,

            // Literal-pool address: a single `ADDI dest, $pc, imm12`
            // (1 instruction). Fixed size (no far-jump-style variability), so
            // worst case == actual.
            Either::Left(AllocatedInstruction::AddrFromLiteralPool(_, _)) => 1,
            // Literal-pool value load: `MOVI` + `ADD` + `LW`/`LB` (3 instructions).
            Either::Left(AllocatedInstruction::LoadFromLiteralPool(_, _)) => 3,
            // A literal pool's size is fixed (raw data), so worst case == actual.
            Either::Left(AllocatedInstruction::LiteralPool(ref entries)) => {
                DataSection::literal_pool_size_bytes(entries) / 4
            }
    ...
    fn instruction_size_not_far_jump(op: &AllocatedAbstractOp, data_section: &DataSection) -> u64 {
        ...
        match op.opcode {
            ...
            // A special case for LoadFromDataSection which may be 1 or 2 ops, depending on the source size.
            Either::Left(AllocatedInstruction::LoadFromDataSection(_, ref data_id)) => {
            ...
            Either::Left(AllocatedInstruction::AddrFromDataSection(_, ref _data_id)) => 2,

            // `AddrFromLiteralPool` lowers to a single `ADDI` (1 instruction),
            // 4 bytes (not size-neutral with `AddrFromDataSection`'s 8). Fixed
            // size: worst case == actual.
            Either::Left(AllocatedInstruction::AddrFromLiteralPool(_, _)) => 1,
            // `LoadFromLiteralPool` lowers to `MOVI` + `ADD` + `LW`/`LB` (3
            // instructions). Not currently generated by `move_to_literal_pools`.
            Either::Left(AllocatedInstruction::LoadFromLiteralPool(_, _)) => 3,
            // A literal pool emits its entries' bytes inline as raw data. Its
            // size in instructions is its byte size / 4 (always word-aligned).
            Either::Left(AllocatedInstruction::LiteralPool(ref entries)) => {
                DataSection::literal_pool_size_bytes(entries) / 4
            }
...
pub(crate) fn compile_jump(
    ...
    if curr_offset > target_offset {
        ...
        return if far {
            ...
            vec![
                RealizedOp {
                    opcode: AllocatedInstruction::LoadFromDataSection(
