...
impl TypeInfo {
    ...
    pub(crate) fn apply_constraints_to_type_argument(
        &self,
        handler: &Handler,
        engines: &Engines,
        ...
        let type_info = engines.te().get(type_argument.type_id);
        let new_type_info = type_info
            .apply_constraints(handler, engines, constraints)
            .unwrap();

        let type_id = engines.te().insert(engines, new_type_info, None);

        TypeArgument {
            type_id,
            initial_type_id: type_argument.initial_type_id,
            span: type_argument.span.clone(),
            call_path_tree: type_argument.call_path_tree.clone(),
        }
    }
    ...
    pub(crate) fn apply_constraints(
        ...
        match self {
            TypeInfo::UnknownGeneric {
                ...
                let mut trait_constraints = trait_constraints.clone();
                for c in constraints {
                    trait_constraints.0.push(c.clone());
                }

                Ok(TypeInfo::UnknownGeneric {
                    name: name.clone(),
                    trait_constraints,
                })
            }
            TypeInfo::Placeholder(t) => {
                let mut trait_constraints = t.trait_constraints.clone();
                for t in constraints {
                    trait_constraints.push(t.clone());
                }
                Ok(TypeInfo::Placeholder(TypeParameter {
                    type_id: t.type_id.clone(),
                    initial_type_id: t.initial_type_id.clone(),
                    name_ident: t.name_ident.clone(),
                    trait_constraints,
                    trait_constraints_span: t.trait_constraints_span.clone(),
                    is_from_parent: t.is_from_parent.clone(),
                }))
            }
            TypeInfo::Array(type_argument, length) => Ok(TypeInfo::Array(
                self.apply_constraints_to_type_argument(
                    handler,
                    engines,
                    constraints,
                    type_argument,
            ...
            TypeInfo::Custom {
                ...
                let Some(type_arguments) = type_arguments.as_ref() else {
                    return Ok(self.clone());
                };
                ...
                Ok(TypeInfo::Custom {
                    qualified_call_path: QualifiedCallPath {
                        call_path: CallPath {
                            prefixes: vec![],
                            suffix: qualified_call_path.call_path.suffix.clone(),
                            is_absolute: false,
                        },
                        qualified_path_root: None,
                    },
                    type_arguments: Some(type_arguments),
                    root_type_id: root_type_id.clone(),
                })
            }
            TypeInfo::Tuple(items) => Ok(TypeInfo::Tuple(
                items
                    .iter()
                    .map(|type_argument| {
                        self.apply_constraints_to_type_argument(
            ...
            TypeInfo::Struct(r) => {
                ...
                Ok(TypeInfo::Custom {
                    qualified_call_path: QualifiedCallPath {
                        call_path: CallPath {
                            prefixes: vec![],
                            suffix: s.call_path.suffix.clone(),
                            is_absolute: false,
                        },
                        qualified_path_root: None,
                    },
                    type_arguments: Some(
                        s.type_parameters
                            .iter()
                            .map(|x| {
                                let t = engines.te().get(x.type_id);
                                let t = t.apply_constraints(handler, engines, constraints).unwrap();
                                let type_id = engines.te().insert(engines, t, None);
                                TypeArgument {
                                    type_id,
