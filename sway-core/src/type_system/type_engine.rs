...
impl TypeEngine {
        ...
        match (self.slab.get(*received), self.slab.get(*expected)) {
            ...
            (
                    ...
                    && a_parameters.len() == b_parameters.len()
                {
                    a_fields.iter().zip(b_fields.iter()).for_each(|(a, b)| {
                        append!(
                            self.unify_left(a.type_id, b.type_id, &a.span, help_text.clone()),
                            warnings,
                            errors
                        );
                    });
            ...
            (Array(a_elem, a_count, _), Array(b_elem, b_count, _)) if a_count == b_count => {
                let (warnings, new_errors) =
                    self.unify_left(a_elem, b_elem, span, help_text.clone());
    ...
    /// Replace any instances of the [TypeInfo::SelfType] variant with
    /// `self_type` in both `received` and `expected`, then unify left
    /// `received` and `expected`.
    fn unify_left_with_self(
        &self,
        received: TypeId,
        expected: TypeId,
        self_type: TypeId,
...
pub(crate) fn unify_left_with_self(
    ...
    TYPE_ENGINE.unify_left_with_self(received, expected, self_type, span, help_text)
}
...
pub(crate) fn unify_left(
    ...
    TYPE_ENGINE.unify_left(received, expected, span, help_text)
}
