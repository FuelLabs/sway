...
use crate::{
    ...
    semantic_analysis::{
        symbol_resolve_context::SymbolResolveContext, type_check_context::EnforceTypeArguments,
        TypeCheckContext,
    },
...
impl SymbolResolveTypeBinding<ParsedDeclId<FunctionDeclaration>> for TypeBinding<CallPath> {
    fn resolve_symbol(
        ...
        let fn_decl = unknown_decl.expect_parsed().to_fn_decl(handler, engines)?;
        Ok(fn_decl)
    }
}
    ...
    for TypeBinding<QualifiedCallPath>
{
    fn resolve_symbol(
        ...
        let mut qualified_call_path_binding = self.clone();
