...
use super::{
    convert::{convert_literal_to_configurable, convert_literal_to_constant},
    function::FnCompiler,
    types::*,
};
...
pub(crate) fn compile_const_decl(
    ...
    match (
        ...
        (None, Some(module_ns)) => {
            ...
            if let Some((name, value, is_configurable)) = decl_params {
                if !is_configurable {
                    ...
                    let const_val = compile_configurable_expression(
                        env.type_engine,
                        env.context,
                        env.md_mgr,
                        env.module,
...
pub(super) fn compile_configurable_expression(
    type_engine: &TypeEngine,
    context: &mut Context,
    md_mgr: &mut MetadataManager,
    module: Module,
    ...
    let configurable_evaluated = compile_configurable_expression_to_configurable(
        type_engine,
        context,
        md_mgr,
        module,
    ...
    Ok(
        Value::new_configurable(context, configurable_evaluated)
            .add_metadatum(context, span_id_idx),
    )
}

pub(crate) fn compile_configurable_expression_to_configurable(
    type_engine: &TypeEngine,
    context: &mut Context,
    md_mgr: &mut MetadataManager,
    module: Module,
    ...
    function_compiler: Option<&FnCompiler>,
    const_expr: &ty::TyExpression,
) -> Result<Configurable, CompileError> {
    let lookup = &mut LookupEnv {
        type_engine,
        context,
        md_mgr,
    ...
    let mut known_consts = MappedStack::<Ident, Configurable>::new();

    config_eval_typed_expr(lookup, &mut known_consts, const_expr)?.map_or(err, Ok)
}

/// Given an environment mapping names to configurables,
/// attempt to evaluate a typed expression to a configurable.
fn config_eval_typed_expr(
    lookup: &mut LookupEnv,
    known_consts: &mut MappedStack<Ident, Configurable>,
    expr: &ty::TyExpression,
) -> Result<Option<Configurable>, CompileError> {
    Ok(match &expr.expression {
        ty::TyExpressionVariant::Literal(l) => Some(convert_literal_to_configurable(l)),
        ty::TyExpressionVariant::FunctionApplication {
            arguments,
            function_decl_id,
            ..
            ...
            for arg in arguments {
                let (name, sub_expr) = arg;
                let eval_expr_opt = config_eval_typed_expr(lookup, known_consts, sub_expr)?;
                if let Some(sub_const) = eval_expr_opt {
                    actuals_const.push((name, sub_const));
                }
            }
            ...
            let body_contents_opt = function_decl.body.contents.last();
            let res = if let Some(first_expr) = body_contents_opt {
                config_eval_typed_ast_node(lookup, known_consts, first_expr)?
            } else {
                None
            };
            for (name, _) in arguments {
        ...
        ty::TyExpressionVariant::VariableExpression { name, .. } => match known_consts.get(name) {
            ...
            Some(cvs) => Some(cvs.clone()),
            None => {
                // 2. Check if name is a global configurable.
                (lookup.lookup)(lookup, name)
                    .ok()
                    .flatten()
                    .and_then(|v| v.get_configurable(lookup.context).cloned())
            }
        },
        ty::TyExpressionVariant::StructExpression { fields, .. } => {
            let (mut field_typs, mut field_vals): (Vec<_>, Vec<_>) = (vec![], vec![]);
            for field in fields {
                let ty::TyStructExpressionField { name: _, value, .. } = field;
                let eval_expr_opt = config_eval_typed_expr(lookup, known_consts, value)?;
                if let Some(cv) = eval_expr_opt {
                    field_typs.push(value.return_type);
                    field_vals.push(cv);
                }
            ...
            if field_vals.len() < fields.len() {
                // We couldn't evaluate all fields to a configurable.
                return Ok(None);
            }
            get_aggregate_for_types(lookup.type_engine, lookup.context, &field_typs)
                .map_or(None, |aggregate| {
                    Some(Configurable::new_struct(&aggregate, field_vals))
                })
        }
        ty::TyExpressionVariant::Tuple { fields } => {
            let (mut field_typs, mut field_vals): (Vec<_>, Vec<_>) = (vec![], vec![]);
        ...
        ty::TyExpressionVariant::Array { contents } => {
            ...
            if element_vals.len() < contents.len() || element_typs.is_empty() {
                // We couldn't evaluate all fields to a configurable or cannot determine element type.
                return Ok(None);
            }
            let mut element_iter = element_typs.iter();
            let element_type_id = *element_iter.next().unwrap();
            ...
            .map_or(None, |aggregate| {
                Some(Configurable::new_array(&aggregate, element_vals))
            })
        }
        ty::TyExpressionVariant::EnumInstantiation {
            enum_decl,
            ...
            let aggregate =
                create_enum_aggregate(lookup.type_engine, lookup.context, &enum_decl.variants);
            if let Ok(aggregate) = aggregate {
                let tag_value = Configurable::new_uint(64, *tag as u64);
                let mut fields: Vec<Configurable> = vec![tag_value];
                match contents {
                    None => fields.push(Configurable::new_unit()),
                    Some(subexpr) => {
                        let eval_expr = config_eval_typed_expr(lookup, known_consts, subexpr)?;
                        eval_expr.into_iter().for_each(|enum_val| {
                            fields.push(enum_val);
                        })
                    }
                }
                Some(Configurable::new_struct(&aggregate, fields))
            } else {
                None
            }
        }
        ty::TyExpressionVariant::StructFieldAccess {
            ...
            resolved_type_of_parent,
            ..
        } => match config_eval_typed_expr(lookup, known_consts, prefix)? {
            Some(Configurable {
                value: ConfigurableValue::Struct(fields),
                ..
            }) => {
                let field_kind = ty::ProjectionKind::StructField {
                    name: field_to_access.name.clone(),
        ...
        ty::TyExpressionVariant::Return(exp) => config_eval_typed_expr(lookup, known_consts, exp)?,
        ty::TyExpressionVariant::ArrayIndex { .. }
        | ty::TyExpressionVariant::IntrinsicFunction(_)
        | ty::TyExpressionVariant::CodeBlock(_)
        | ty::TyExpressionVariant::Reassignment(_)
...
fn config_eval_typed_ast_node(
    ...
    match &expr.content {
        ...
        ty::TyAstNodeContent::Expression(e) | ty::TyAstNodeContent::ImplicitReturnExpression(e) => {
            config_eval_typed_expr(lookup, known_consts, e)
        }
        ty::TyAstNodeContent::SideEffect => Ok(None),
    }
}
