...
impl<'eng> FnCompiler<'eng> {
    ...
    fn compile_storage_read(
        ...
        match ty {
            ty if ty.is_struct(context) && !self.experimental_storage => {
                let temp_name = self.lexical_map.insert_anon();
                let struct_var = self
                    .function
            ...
            _ => {
                ...
                if self.experimental_storage {
                    ...
                    // The type of a storage access is `StorageHandle` which is a struct containing
                    // a `b256` and ` u64`.
                    let storage_handle_aggregate = Type::new_struct(
                        context,
                        vec![Type::get_b256(context), Type::get_uint64(context)],
                    );

                    // Local variable holding the `StorageHandle` struct
                    let storage_handle_local_name = self.lexical_map.insert_anon();
                    let storage_handle_ptr = self
                        .function
                        .new_local_var(
                            context,
                            storage_handle_local_name,
                            storage_handle_aggregate,
                            None,
                        )
                        .map_err(|ir_error| {
                            CompileError::InternalOwned(ir_error.to_string(), Span::dummy())
                    ...
                    let mut storage_handle = self
                        .current_block
                        .ins(context)
                        .get_local(storage_handle_ptr)
                        .add_metadatum(context, span_md_idx);

                    // Insert the key as the first field in the `StorageHandle` struct
                    storage_handle = self
                        .current_block
                        .ins(context)
                        .insert_value(storage_handle, storage_handle_aggregate, key_val, vec![0])
                        .add_metadatum(context, span_md_idx);

                    // Insert the offset as the second field in the `StorageHandle` struct
                    let offset_within_slot_val =
                        Constant::get_uint(context, 64, offset_within_slot);
                    storage_handle = self
                        .current_block
                        .ins(context)
                        .insert_value(
                    ...
                    match ty.get_content(context) {
                        ...
                        TypeContent::Bool | TypeContent::Uint(_) => self
                            .compile_uint_or_bool_storage_read(context, &key_val, ty, span_md_idx),
