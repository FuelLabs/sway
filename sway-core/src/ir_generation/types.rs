use crate::{
    ...
    type_system::{check_type_is_not_unknown, TypeId, TypeInfo},
};
...
pub(super) fn get_struct_name_field_index_and_type(
    ...
    let ty_info = check_type_is_not_unknown(field_type, &field_kind.span()).ok()?;
    ...
    fields
        ...
        .try_fold(
            ...
            |(mut fld_idcs, prev_type_id), field| {
                ...
                let ty_info = match check_type_is_not_unknown(prev_type_id, &field_kind.span()) {
                    Ok(ty_info) => ty_info,
                    Err(error) => {
