...
sway_apps_branch_name="benchmarks"
...
# push the benchmark data to a repo if requested
if $PUSH_DATA; then
    sway_peformance_data_dir=performance-data
    sway_performance_data_repo_url=https://github.com/FuelLabs/sway-performance-data.git
    ...
    cp -r $benchmarks_dir/* "$SCRIPT_DIR/$sway_peformance_data_dir/"

    echo "Pushing data to sway-performance-data repository..."
    git -C "$SCRIPT_DIR/$sway_peformance_data_dir/" add .
    git -C "$SCRIPT_DIR/$sway_peformance_data_dir/" commit -m "Update performance data"
    git -C "$SCRIPT_DIR/$sway_peformance_data_dir/" push
fi
