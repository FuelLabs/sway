...
pub fn print_diff(proj_name: Option<&str>, diff: &Diff) {
    print_removed_pkgs(proj_name, diff.removed.iter().cloned());
    print_added_pkgs(proj_name, diff.added.iter().cloned());
}

pub fn print_removed_pkgs<'a, I>(proj_name: Option<&str>, removed: I)
where
...
pub fn print_added_pkgs<'a, I>(proj_name: Option<&str>, removed: I)
where
    I: IntoIterator<Item = &'a PkgLock>,
{
    for pkg in removed {
