...
pub fn dependency_namespace(
    ...
    compiled_contract_deps: &HashMap<NodeIx, CompiledContractDependency>,
    graph: &Graph,
    node: NodeIx,
...
pub fn compile(
    ...
    let ast_res = time_expr!(
        "compile to ast",
        compile_ast(pkg, profile, engines, namespace)?
    );
    ...
    let mut program_abi = time_expr!(
        "generate JSON ABI program",
        match asm_res.value.as_ref() {
            Some(asm) => sway_core::abi_generation::program_abi(
                &pkg.target,
                typed_program,
                engines.te(),
                &asm.0
...
pub fn build(
        ...
        .filter(|node| required.contains(node))
    {
        ...
        if let TreeType::Library { ref name } = compiled.tree_type {
