...
",
    ...
    #[test]
    fn identity_arithmetic_ops_are_preserved() {
        // These are value identities, but the corresponding FuelVM arithmetic
        // instructions also clear arithmetic status registers such as `$of`.
        // Removing them changes observable program state, so constant folding
        // must leave the operations in place when one operand is dynamic.
        for (opcode, identity) in [("add", "0"), ("mul", "1"), ("div", "1"), ("sub", "0")] {
            let body = format!(
                "entry fn main() -> u64 {{
                    local u64 LOCAL
                entry():
                    identity = const u64 {identity}, !0
                    l_ptr = get_local __ptr u64, LOCAL, !0
                    l = load l_ptr, !0
                    result = {opcode} l, identity, !0
                    ret u64 result, !0
                }}"
            );
            assert_optimization(&[CONST_FOLDING_NAME], &body, None::<[&str; 0]>);
        }
    }
}
