...
impl fmt::Display for IrError {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        match self {
            ...
            IrError::VerifySwitchDuplicateCase(duplicate_value) => {
                write!(
                    f,
                    "Verification failed: Switch contains duplicate case value {duplicate_value}."
                )
            }
