            ...
            TypedExpression {
                expression: TypedExpressionVariant::FunctionApplication {
                    ...
                    selector: if method.is_contract_call {
                        let contract_address = match contract_caller.map(|x| x.return_type) {
                            Some(MaybeResolvedType::Resolved(ResolvedType::ContractCaller {
                                address,
                                ..
                            })) => address,
                            _ => {
                        ...
                        let func_selector =
                            type_check!(method.to_fn_selector_value(), [0; 4], warnings, errors);
