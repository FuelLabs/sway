...
pub(crate) fn to_string(context: &Context) -> String {
    context
        .modules
        .iter()
...
fn module_to_doc<'a>(context: &'a Context, module: &'a ModuleContent) -> Doc {
    Doc::line(Doc::Text(format!(
        "{} {} {{",
        match module.kind {
            Kind::_Contract => "contract",
            Kind::_Library => "library",
            Kind::_Predicate => "predicate ",
            Kind::Script => "script",
        },
        &module.name
    )))
        ...
        ValueContent::Instruction(instruction) => match instruction {
            ...
            Instruction::Phi(pairs) => {
                if pairs.is_empty() {
                    ...
                    Doc::line(
                        Doc::text(format!("{} = phi", namer.name(context, ins_value))).append(
                    ...
                    |AsmInstruction {
                        ...
                        Doc::line(
                            Doc::text(format!("{:6} ", name)).append(
                                Doc::list_sep(
                                    args.iter().map(|arg_str| Doc::text(arg_str)).collect(),
                                    Doc::Comma,
                                )
