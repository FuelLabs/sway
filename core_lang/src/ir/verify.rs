...
impl Context {
    ...
    fn verify_block(&self, function: &FunctionContent, block: &BlockContent) -> Result<(), String> {
        ...
        // We can have a strict version of this check which insists on having no dead code -- i.e.,
        // a single terminator at the very end.  But before the dead code elimination pass this
        // obviously may not be true.  So here we just insist the final instruction is a
        // terminator.
        let (last_is_term, _num_terms) =
        ...
        if !last_is_term {
            Err(format!(
                ...
                "Block {} must have a terminator as its last instruction.",
                block.label
            ))
        } else {
            Ok(())
    ...
    fn verify_ret(
        ...
        if &function.return_type != ty {
            Err(format!(
                "Function {} return type must must match ret instructions.",
                function.name
            ))
        // XXX When we have some type info available from instructions...
