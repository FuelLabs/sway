...
pub(crate) fn inline_all_function_calls(
    context: &mut Context,
    function: &Function,
) -> Result<bool, String> {
...
pub(crate) fn inline_function_call(
    ...
    // old return value from the inlined function, and then remove the call altogether.
    post_block.replace_value(context, call_site, post_block.get_phi(context));
    ...
    if let ValueContent::Instruction(old_ins) = context.values[instruction.0].clone() {
        let new_ins = match old_ins {
            ...
            Instruction::ExtractValue {
                ...
                .ins(context)
                .extract_value(map_value(aggregate), ty, indices.clone()),
